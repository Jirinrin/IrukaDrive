...
namespace Shapes {
	...
	public static partial class Draw {
		...
		[OvldGenCallTarget] public static void Polygon( [OvldDefault( nameof(BlendMode) )] ShapesBlendMode blendMode,
			...
			matPolygon.SetColor( ShapesMaterialUtils.propColor, useFill ? fill.colorStart : color );
			matPolygon.SetInt( ShapesMaterialUtils.propFillType, fill.GetShaderFillModeInt() );
			if( useFill ) {
				matPolygon.SetInt( ShapesMaterialUtils.propFillSpace, (int)fill.space );
				matPolygon.SetVector( ShapesMaterialUtils.propFillStart, fill.GetShaderStartVector() );
				matPolygon.SetColor( ShapesMaterialUtils.propColorEnd, fill.colorEnd );
				if( fill.type == FillType.LinearGradient )
					matPolygon.SetVector( ShapesMaterialUtils.propFillEnd, fill.linearEnd );
			}
